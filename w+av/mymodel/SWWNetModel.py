...
class SWWNetModel(nn.Module):
    def __init__(self):
        ...
        self.positionalEncoding = PositionalEncoding(dModel=512, maxLen=900)
    ...
    def forward(self, audioFeature, visualFeature,mask):
        ...
        videoEmbed = visualFeature
        if not audioEmbed.shape[1] == videoEmbed.shape[1]:
            videoEmbed = videoEmbed[:,0:-1,:]
            print(audioEmbed.shape,videoEmbed.shape)
        ...
        videoEmbed = visualFeature.transpose(0,1)

        audioEmbed = self.crossA2V(audioEmbed,videoEmbed)
        videoEmbed = self.crossA2V(audioEmbed,videoEmbed)
        audioEmbed = audioEmbed.transpose(0,1)
        videoEmbed = videoEmbed.transpose(0,1)
        '''




        out_wake_word = self.forward_audio_backend(audioEmbed,mask)
        out_wake_word2 = self.forward_video_backend(videoEmbed, mask)
        return out_wake_word,out_wake_word2
