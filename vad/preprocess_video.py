...
def preprocess_video(len):
    ...
    vf = visualFrontend()
    model_dict = vf.state_dict()
    pretrained_dict = torch.load(r'C:\Users\WGQ\Desktop\qtchoose\static/visual_frontend.pt')
    pretrained_dict = {k: v for k, v in pretrained_dict.items() if k in model_dict}
    model_dict.update(pretrained_dict)
    vf.load_state_dict(model_dict)
    vf.to(device)


    inp1 = np.load(r'C:\Users\WGQ\Desktop\qtchoose\static/video/1.npy')

    list = []
    if len==3:
        inp1 = inp1.reshape(3,inp1.shape[0]//3,inp1.shape[1],inp1.shape[2])
        for i in range(3):
            inp = inp1[i, :, :, :] / 255
            inp = np.expand_dims(inp, axis=1)
            inp = np.expand_dims(inp, axis=1)
            inp = (inp - 0.4161) / 0.1688
        ...
        kkk = np.stack(list, axis=0)
        np.save(r'C:\Users\WGQ\Desktop\qtchoose\static/video/1.npy', kkk)

    if len==2:
        inp1 = inp1.reshape(2, inp1.shape[0] // 2, inp1.shape[1], inp1.shape[2])
        for i in range(2):
            inp = inp1[i, :, :, :] / 255
            inp = np.expand_dims(inp, axis=1)
            inp = np.expand_dims(inp, axis=1)
        ...
        list[0] = torch.tensor(list[0])
        print(list[0].shape)
        list[1] = torch.tensor(list[1])
        list[2] = torch.tensor(list[2])
        kkk = [list[0],list[1],list[2]]
        aaa = pad_sequence(kkk, batch_first=True, padding_value=0)
        aaa = aaa.numpy()
        np.save(r'C:\Users\WGQ\Desktop\qtchoose\static/video/1.npy', aaa)
