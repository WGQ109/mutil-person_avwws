...
def main():
    ...
    dset_loaders = {x: torch.utils.data.DataLoader(dsets[x], batch_size=8, shuffle=True, num_workers=0,drop_last=True,
                                                   collate_fn=mycollate_fn) for x in ['train', 'val']}
    model = SWWNetModel()
    model = model.to(device)

    optim = torch.optim.Adam(model.parameters(), lr=0.0001, betas=(0.9, 0.999),weight_decay=1e-4)
    # optim = torch.optim.SGD(model.parameters(), lr=1e-3, momentum=0.9, weight_decay=0.001)  # 定义优化器
    ...
    while(1):
        ...
        if ((epoch % 2 == 0) or (epoch == 1000)) and (epoch != 0) and epoch>8:
