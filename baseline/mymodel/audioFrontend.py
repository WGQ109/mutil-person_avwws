...
class ResNet(nn.Module):
    ...
    def forward(self, inputBatch):
        ...
        inputBatch = inputBatch.reshape(inputBatch.shape[0] * inputBatch.shape[1], 1,inputBatch.shape[2])
        inputBatch = self.fronted1D(inputBatch)
...
if __name__ == '__main__':
    ...
    import numpy as np
    A = np.random.randint(15, size=(2,3,2,4))
    A = torch.from_numpy(A)
    A = A.float()
    print(A)
    l,attention = torch.max(A, dim=0)
    print(l)
